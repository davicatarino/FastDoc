...
async function transformTextToStructuredData(text) {
    const response = await openai.chat.completions.create({
        messages: [{
            role: "system",
            content: `Extraia os lançamentos (histórico de compras no cartão) com data, nome e valor, e retorne como um objeto JSON com três arrays: "data", "nome" e "valor".\n\n${text}`
        }],
        model: "gpt-3.5-turbo",
        max_tokens: 500,
        temperature: 0.5
    ...
    console.log('Structured Data:', structuredData); // Adiciona depuração
...
async function addDataToExcel(data, filename) {
    ...
    if (worksheet.actualRowCount === 0) {
        worksheet.addRow(['Data', 'Nome', 'Valor']);
    }

    // Adiciona as novas linhas
    const rows = data.data.map((date, index) => ({
        Data: date,
...
app.post('/upload', upload.single('file'), async (req, res) => {
    ...
    const excelFilename = path.join(__dirname, './planilha/testes.xlsx'); // Substitua pelo nome do seu arquivo Excel
    ...
    try {
        ...
        const extractedText = conversionResponse.data.body;
